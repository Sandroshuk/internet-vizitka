const examples = {
  ...
  hair: {
    ...
    story: "Диана просит прислать текущее фото волос и пример желаемого результата. Так клиент сразу понимает время, бюджет и реальность оттенка.",
  ...
  speech: {
    ...
    lead: "Диагностика, занятия и спокойный контакт для родителей без поиска информации в переписке.",
    accent: "#87ceeb",
    bg: "#fff0f5",
    ...
    works: [
      ["Диагностика", "первая встреча и план"],
      ["Звуки", "упражнения под ребенка"],
      ["Родителям", "что делать дома"],
    ],
    story: "Алена принимает детей и родителей в спокойном ритме. После первой встречи семья получает понятный план: над чем работаем и как заниматься дома.",
  ...
  transport: {
    ...
    story: "Олег просит прислать адреса, этажи, лифт и пару фото груза. После этого сразу называет машину, время и примерную стоимость.",
  ...
  repair: {
    ...
    story: "Максим работает по небольшим бытовым задачам: повесить, собрать, заменить, закрепить. Клиент заранее видит, что можно прислать в заявку.",
  ...
  network: {
    ...
    lead: "Витрина с подбором товаров для здоровья и красоты, отзывами и быстрым первым сообщением.",
    accent: "#4caf8c",
    bg: "#fcfbe3",
    ...
    works: [
      ["Подбор", "здоровье, уход, дом"],
      ["Каталог", "понятные категории"],
      ["Отзывы", "реальные впечатления клиентов"],
    ],
    story: "Орбита собирает в одном месте товары для здоровья, красоты и дома. По ссылке человек видит направления, примеры наборов, отзывы и может сразу написать для подбора.",
